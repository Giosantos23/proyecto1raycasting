...
pub fn render_3d(
    ...
    for i in 0..num_rays {
        ...
        let wall_height = ((framebuffer.height as f32 / distance) * 1.2) as usize; // 1.5 es un ejemplo
        ...
        for y in wall_start..wall_end {
            ...
            framebuffer.point_with_color(i, y, Color(color)); // Aplica el color obtenido de la textura
        }
    }
}
...
fn cast_ray_3d(maze: &Vec<Vec<char>>, player: &Player, a: f32, block_size: usize) -> (f32, usize, usize) {
    ...
    loop {
        ...
        if x >= maze[0].len() * block_size || y >= maze.len() * block_size {
            break (d, x, y);  // Si se sale del mapa, retorna la distancia y la posición
        }
        ...
        if maze[i][j] != ' ' {
            break (d, x % block_size, y % block_size);  // Si choca con una pared, retorna la distancia y la posición relativa al bloque
        }

        d += step_size; // Aumenta la precisión
    }
}
