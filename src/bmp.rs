...
const BMP_BITS_PER_PIXEL: u16 = 24; 
...
fn write_bmp_header(file: &mut BufWriter<File>, width: usize, height: usize) -> std::io::Result<()> {
    let file_size = (height * width * (BMP_BITS_PER_PIXEL as usize / 8)) + BMP_HEADER_SIZE;
...
fn write_pixel_data(file: &mut BufWriter<File>, buffer: &[u32], width: usize, height: usize) -> std::io::Result<()> {
    let padding_size = (4 - (width * BMP_BITS_PER_PIXEL as usize / 8) % 4) % 4; 
    let padding = [0u8; 3]; 
...
/// Trait para escribir un framebuffer como un archivo BMP
pub trait WriteBmp {
    fn render_buffer(&self, file_path: &str) -> std::io::Result<()>;
}

/// Implementación del trait WriteBmp para Framebuffer
